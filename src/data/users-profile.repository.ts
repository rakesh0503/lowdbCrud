...
export class UserProfileRepository {
    ...
    constructor() {
        ...
        // const __dirname = dirname(fileURLToPath("file:///C:/git/sp-models/src/data/user-profiles.repository.ts"))
        // "file:///C:/git/sp-models/src/data/user-profiles.repository.ts"
    ...
    async getById(id: number): Promise<UserProfile | undefined> {
        await this.db.read();
        return this.db.data.usersProfile.find((userProfile: { id: number; }) => userProfile.id === id);
    }
    ...
    async delete(id: number): Promise<void> {
        await this.db.read();
        this.db.data.usersProfile = this.db.data.usersProfile.filter((userProfile) => userProfile.id !== id);
        await this.db.write(); // Make sure to write after modifying data
    }
}
